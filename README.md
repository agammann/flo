...
The separate [AWS staging website](https://i4ceh4qpdg.execute-api.us-west-2.amazonaws.com/) is deployed. Real Amazon sign-in has been exercised; the authenticated-unlinked UI keeps repair information blocked pending shop verification. On September 6, the corrected request service created one short-lived pending request through the hosted browser flow, with HTTP 200 and 12 successful post-deployment checks. See [deployment and test evidence](docs/verification/request-dynamodb-kms-deployment-2026-09-06.md). This is request-stage success, not completed customer linking: private approval/redemption remain disabled, no customer was linked by that test, and real-customer ownership verification remains incomplete. Website sessions and AWS service credentials cannot unlock the website's closed Alexa routes. Follow the [identity architecture and controlled setup guide](docs/architecture/customer-identity.md); do not expose the shop demo or raw mock APIs. This is not a certification-ready release.
