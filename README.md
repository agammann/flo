...
The separate [AWS staging website](https://i4ceh4qpdg.execute-api.us-west-2.amazonaws.com/) is deployed. Real Amazon sign-in has been exercised; the authenticated-unlinked UI keeps repair information blocked pending shop verification. The [September 8 pairing test](docs/verification/fictional-a-pairing-completed-2026-09-08.md) completed a separately authorized fictional-customer designation, private operator approval and explicit customer redemption. The temporary operator grant was removed and subsequent invocation denial verified. The [hosted fixture test](docs/verification/customer-repair-fixture-live-2026-09-08.md) then verified A-only repair access, B/unknown denial, absent-estimate handling and logout. No second Amazon account or real-world repair-ownership proof was tested. Hosted fixtures are independent of local shop data. Website sessions and AWS service credentials cannot unlock the website's closed Alexa routes. Follow the [identity architecture and controlled setup guide](docs/architecture/customer-identity.md); do not expose the shop demo or raw mock APIs. This is not a certification-ready release.
...
This diagram is the larger deployment target, not a deployed architecture. DynamoDB already supports the narrator allowance and the separate hosted customer's auth/session, trusted-link, enrollment and repair-projection storage. Secrets Manager supplies private staging runtime configuration. AgentCore and durable shop work-order/estimate/purchase/schedule state remain future work. Local shop business state remains in seeded, in-memory stores; it is not synchronized to the hosted customer projections. See [the AWS architecture](docs/architecture/aws.md) for dated deployment evidence and the live/future boundary.
