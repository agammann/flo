...
Current staging supplement, September 8, 2026: the durable Lambda/DynamoDB website and real LWA configuration are deployed. Actual hosted sign-in/sign-out, authenticated-unlinked UI, private fictional-customer approval/redemption and A-only repair reads with B/unknown denial have been exercised. See [completed fictional pairing](../verification/fictional-a-pairing-completed-2026-09-08.md) and [hosted isolation evidence](../verification/customer-repair-fixture-live-2026-09-08.md). The tested designation is fictional, not proof of real-world repair ownership; no second Amazon account was tested. The loopback/file/in-memory sections below describe the earlier local implementation, not the deployed storage model.

Earlier implementation checkpoint: local simulated-provider contract and HTTP/MCP tests. The current supplement above supersedes that checkpoint for hosted login and the controlled fictional pairing/isolation checks. **Real-world repair enrollment, official Alexa+ linking and certification are not verified.** This remains a limited, read-only staging website, not a production customer service.
...
3. For this standalone file-backed entrypoint, supply `LWA_ENABLED=true`, `LWA_CLIENT_ID`, `LWA_CLIENT_SECRET`, `FLO_CUSTOMER_PUBLIC_ORIGIN`, and `FLO_CUSTOMER_LINKS_FILE` through private runtime configuration. Never put the secret into browser code, chat, screenshots or Git, and never fetch secret plaintext into an agent tool response. The separate AWS staging deployment uses Secrets Manager and DynamoDB-backed links; follow its deployment instructions rather than treating this local file-backed example as the hosted configuration. See the [AWS architecture checkpoint](aws.md).
