...
import { estimateView } from './estimate-view.js';
...
function render(view, data) {
  if (view === "work-order") {
    ...
    const detail = estimateView(estimate);
    const price = cents => Number.isSafeInteger(cents) && cents >= 0 ? dollars(cents) : 'Unavailable';
    canvas.innerHTML = `<div class="summary-card"><span class="eyebrow">Estimate</span><h3>Repair estimate</h3><p>${escapeHtml(detail.description)}</p></div><div class="money-list"><div class="money-row"><span>Part customer price</span><strong>${price(detail.partsCents)}</strong></div><div class="money-row"><span>Labor</span><strong>${price(detail.laborCents)}</strong></div><div class="money-row"><span>Shop supplies</span><strong>${price(estimate.feesCents)}</strong></div><div class="money-row"><span>Tax</span><strong>${price(estimate.taxCents)}</strong></div><div class="money-row total"><span>Total</span><strong>${price(estimate.totalCents)}</strong></div></div>`;
