...
export function buildEnrollmentRuntimeTemplate(baselines) {
  ...
  for (const [kind, cap] of [["request", "Request"], ["redemption", "Redemption"], ["approval", "Approval"]]) {
    ...
    const tokenConfig = { account: "114599789754", region: "us-west-2", tables: Object.fromEntries(Object.keys(tableParameters).map(k => [k, `TOKEN_${k}`])),
      logGroup: `/aws/lambda/TOKEN_${kind}`, ...(kind === "request" ? { redemptionVersionArn: "arn:aws:lambda:us-west-2:114599789754:function:TOKEN_redemption:1" } : {}) };
    ...
    else Object.assign(variables, { FLO_ENROLLMENT_ENABLED: ref("EnableEnrollment"), FLO_AWS_ACCOUNT_ID: ref("AWS::AccountId"), LWA_CLIENT_ID: ref("LwaClientId"),
    ...
    Resources[`${cap}Function`] = { Type: "AWS::Lambda::Function", DependsOn: [`${cap}Logs`, `${cap}Baseline`], Properties: {
      FunctionName: sub(name), Runtime: "nodejs22.x", Handler: "index.handler", Architectures: ["x86_64"], Role: attr(`${cap}Role`, "Arn"),
      ...
      Environment: { Variables: variables }, LoggingConfig: { LogFormat: "JSON", ApplicationLogLevel: "WARN", SystemLogLevel: "WARN" }
    } };
  ...
  for (const [id, method, path] of [["PairingPage", "GET", "pairing"], ["PairingAsset", "GET", "pairing.js"], ["StartPairing", "POST", "enrollment/request"], ["RedeemPairing", "POST", "enrollment/redeem"]]) {
    Resources[`${id}Permission`] = { Type: "AWS::Lambda::Permission", Condition: "ExposeRoutes", Properties: {
    ...
    Resources[`${id}Route`] = { Type: "AWS::ApiGatewayV2::Route", Condition: "ExposeRoutes", DependsOn: [`${id}Permission`], Properties: {
  ...
  return { AWSTemplateFormatVersion: "2010-09-09", Description: "Flo us-west-2 enrollment runtime REVIEW ONLY. No repair data, operator grants, customer links or replacement website. Explicit approval required.",
    ...
    Parameters, Conditions: { ExposeRoutes: { "Fn::And": [{ "Fn::Equals": [ref("PublishRoutes"), "true"] }, { "Fn::Equals": [ref("EnableEnrollment"), "true"] }] } },
